...
# Create PR to merge back into develop with no PR body 
echo -e "\033[34mCreating PR for merging back into develop\033[0m"
gh pr create \
  -H "${release_branch}" \
  -B "develop" \
  -b "Merge hotfix changes back into develop" \
  -t "build: merge ${release_version} back to develop"
