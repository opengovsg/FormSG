...
db.forms.update(
  ...
  {
    $set: {
      'form_fields.$[elem].dateValidation.selectedDateValidation':
        'Disallow past dates',
    },
  },
...
const afterScriptWithDateValidationDisallowPastDates = [
  ...
  {
    $match: {
      'form_fields.dateValidation.selectedDateValidation':
        'Disallow past dates',
    },
  },
  { $count: 'numFormFields' },
...
db.getCollection('forms').aggregate(
  afterScriptWithDateValidationDisallowPastDates,
)
...
const afterScriptWithDateValidationSelectedDateVal = [
  ...
  {
    $match: {
      'form_fields.dateValidation.selectedDateValidation': { $exists: true },
    },
  },
  { $count: 'numFormFields' },
]
...
db.getCollection('forms').aggregate(
  afterScriptWithDateValidationSelectedDateVal,
)
