...
let formsWithMobileWa_0 = db
  .getCollection('forms')
  .find({ 'form_fields.fieldType': 'mobile_wa' })
  .map((x) => x._id)
db.getCollection('submissions').count()
db.getCollection('submissions').count({ form: { $in: formsWithMobileWa_0 } })
db.getCollection('submissions').deleteMany({
  form: { $in: formsWithMobileWa_0 },
})
db.getCollection('submissions').count()
...
let formsWithMobileWa_1 = db
  .getCollection('forms')
  .find({ 'form_fields.fieldType': 'mobile_wa' })
  .map((x) => x._id)
db.getCollection('formStatisticsTotal').count()
db.getCollection('formStatisticsTotal').count({
  formId: { $in: formsWithMobileWa_1 },
})
db.getCollection('formStatisticsTotal').deleteMany({
  formId: { $in: formsWithMobileWa_1 },
})
db.getCollection('formStatisticsTotal').count()
...
let formsWithMobileWa_2 = db
  .getCollection('forms')
  .find({ 'form_fields.fieldType': 'mobile_wa' })
  .map((x) => x._id)
db.getCollection('formStatisticsDaily').count()
db.getCollection('formStatisticsDaily').count({
  formId: { $in: formsWithMobileWa_2 },
})
db.getCollection('formStatisticsDaily').deleteMany({
  formId: { $in: formsWithMobileWa_2 },
})
db.getCollection('formStatisticsDaily').count()
