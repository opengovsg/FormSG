...
{
  db.forms.count({
    'startPage.logo.fileId': { $eq: '1622549643061-sp%20student%20logo.png' },
  })
}

// UPDATE
{
  let spStudentUserIds = db.users
    .find({ email: /.+ichat\.sp\.edu\.sg$/i })
    .map((b) => b._id)
  let spStudentFormIds = db.forms
    .find({ admin: { $in: spStudentUserIds } })
    .map((b) => b._id)
