...
describe('Form Model', () => {
  ...
  describe('Statics', () => {
    ...
    describe('getFullFormById', () => {
      ...
      describe('allowedEmailDomains correction', () => {
        it('should lowercase allowedEmailDomains values in a verifiable email field', async () => {
          const form = await Form.create({
            ...
            form_fields: [
              generateDefaultField(BasicField.Email, {
                isVerifiable: true,
                allowedEmailDomains: ['@foo.com', '@BAR.COM', '@BaZ.CoM'],
              }),
            ],
          })

          const readData = (await Form.getFullFormById(form._id)).toObject()

          expect(readData.form_fields[0].allowedEmailDomains).toEqual([
            '@foo.com',
            '@bar.com',
            '@baz.com',
          ])
        ...
        it('should lowercase allowedEmailDomains values in all verifiable email fields', async () => {
          const form = await Form.create({
            ...MOCK_EMAIL_FORM_PARAMS,
            admin: populatedAdmin,
            form_fields: [
              ...
              generateDefaultField(BasicField.Email, {
                isVerifiable: true,
                allowedEmailDomains: ['@shoe.com', '@BOAT.COM', '@CaR.CoM'],
              }),
            ],
          })
          ...
          expect(readData.form_fields[1].allowedEmailDomains).toEqual([
            '@shoe.com',
            '@boat.com',
            '@car.com',
          ])
        ...
        it('should not affect non-email fields', async () => {
          const form = await Form.create({
            ...MOCK_EMAIL_FORM_PARAMS,
            admin: populatedAdmin,
            form_fields: [
              generateDefaultField(BasicField.Decimal),
              generateDefaultField(BasicField.Email, {
                isVerifiable: true,
                allowedEmailDomains: ['@foo.com', '@BAR.COM', '@BaZ.CoM'],
              }),
          ...
          const readForm = (await Form.getFullFormById(form._id)).toObject()

          expect(readForm.form_fields.length).toEqual(4)

          expect(readForm.form_fields[0].fieldType).toEqual('decimal')
          expect(readForm.form_fields[1].fieldType).toEqual('email')
          expect(readForm.form_fields[2].fieldType).toEqual('date')
          expect(readForm.form_fields[3].fieldType).toEqual('mobile')

          expect(readForm.form_fields[1].allowedEmailDomains).toEqual([
            '@foo.com',
            '@bar.com',
            '@baz.com',
          ])
