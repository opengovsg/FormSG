/**
 * Handler tests, run by `node --test` rather than jest: this service ships to
 * Lambda with exactly one dependency, and Node 22 is its own runtime.
 *
 * The handler reads its configuration at module load, so each test resets the
 * require cache and loads it afresh.
 */
...
test('throws when the SSM parameter is empty, before any sweep', async () => {
  ...
  // An empty secret would 401 every sweep, which reads as a quiet run.
