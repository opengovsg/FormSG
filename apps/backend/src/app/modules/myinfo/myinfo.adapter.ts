...
import { CHILD_RECORD_TYPE_LABEL } from 'formsg-shared/constants/field/myinfo'
import {
  ChildRecordType,
...
> {
  ...
  getChildrenBirthRecords(
    ...
    // Informational only (see ChildRecordType) — mirrors the same
    // leading-birth-records-then-sponsored order as every other column, so
    // it stays index-aligned with them. Converted to its display label here
    // (matching every other column, e.g. ChildGender's `.desc`), so nothing
    // downstream — including the SDK, which cannot depend on formsg-shared's
    // CHILD_RECORD_TYPE_LABEL — needs to know the internal enum exists.
    const birthRecordCount =
      (this.#personData.childrenbirthrecords as Array<unknown> | undefined)
        ?.length ?? 0
    const sponsoredRecordCount = includeSponsoredChildren
      ? ((
          this.#personData.sponsoredchildrenrecords as
            | Array<unknown>
            | undefined
        )?.length ?? 0)
      : 0
    result.type = [
      ...Array<string>(birthRecordCount).fill(
        CHILD_RECORD_TYPE_LABEL[ChildRecordType.Nuclear],
      ),
      ...Array<string>(sponsoredRecordCount).fill(
        CHILD_RECORD_TYPE_LABEL[ChildRecordType.Sponsored],
      ),
    ]
