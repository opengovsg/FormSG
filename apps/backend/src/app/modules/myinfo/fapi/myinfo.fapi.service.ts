...
const withConfig = <T, E>(
  ...
  return ResultAsync.fromPromise(getConfiguration(), (error) => {
    logger.error({
      message: 'Failed to initialise MyInfo FAPI client',
      meta: { action: 'configuration' },
      error,
    })
    return new MyInfoFapiConfigError()
  }).andThen((config) => ResultAsync.fromPromise(run(config), onError))
}
