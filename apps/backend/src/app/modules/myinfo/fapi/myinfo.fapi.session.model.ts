...
>(
  {
    // Opaque, so the value in the cookie carries no ordering or timing signal.
    ...
    // Pseudonymous under FAPI, so stored in the clear.
    sub: optionalString,
...
MyInfoFapiSessionSchema.statics.createPending = async function (
  ...
  const created = await this.create({
    ...
    dpopPrivateJwkEnc: encryptJwk(session.dpopPrivateJwk),
...
MyInfoFapiSessionSchema.statics.loadForCallback = async function (
  ...
  return {
    ...
    exchange: {
      ...
      nonce: session.nonce,
      codeVerifier: session.codeVerifier,
      dpopPrivateJwk: decryptJwk(session.dpopPrivateJwkEnc),
    },
  }
}
...
MyInfoFapiSessionSchema.statics.consumeExchanged = async function (
  ...
  return {
    formId: session.formId,
    accessToken: decrypt(session.accessTokenEnc),
    sub: session.sub,
    dpopPrivateJwk: decryptJwk(session.dpopPrivateJwkEnc),
  }
}
