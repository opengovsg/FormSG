 ...
 *
 * The authorization code is exchanged here: it is single-use, expires in 60
 * seconds, and is bound to a DPoP key that only the session document holds.
 * Login failures still redirect to the form with the session cookie intact, so
 * form load raises ErrorCode.myInfo — the same outcome as a spent v3 auth code.
 */
...
> = async (req, res) => {
  ...
  const logMeta = {
    action: 'loginToMyInfoFapi',
    sessionId,
  }

  let session: MyInfoFapiCallbackSession | null
  try {
    session = await MyInfoFapiSession.loadForCallback(sessionId)
  } catch (error) {
    logger.error({
...
const redirectDestination = ({
  ...
  // For local dev the frontend is served from a different origin to the backend.
  const base =
