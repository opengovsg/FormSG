...
describe('multirespondent-submission.utils', () => {
  ...
  describe('getMrfVersion', () => {
    ...
    it('pins generic and zapier to V3 for every write-guard value', () => {
      for (const webhookType of ['generic', 'zapier'] as const) {
    ...
    it('with the write-guard flag off, reduces exactly to hasWebhook ? 1 : 2', () => {
      const consumerClasses: WebhookType[] = [
        undefined,
        'plumber',
        'generic',
        'zapier',
      ]
      for (const webhookType of consumerClasses) {
        const hasWebhook = webhookType !== undefined
        expect(
          getMrfVersion({ webhookType, isStepWriteTokenEnabled: false }),
        ).toBe(hasWebhook ? 1 : 2)
      }
    })
  })
